...
class CmdConsole
  class Command
    ...
    class << self
      ...
      def name
        super.to_s == "" ? "#<class(CmdConsole::Command #{match.inspect})>" : super
      end
      ...
      # @param [Hash] options Behavioral options (see {CmdConsole::CommandSet#command})
      # @param [Module] helpers A module of helper functions to be included.
      # @yield optional, used for BlockCommands
      # @return [Class] (a subclass of {CmdConsole::Command})
      ...
      def group(name = nil)
        @group ||= begin
          name || case CmdConsole::Method(block).source_file
    ...
    # Properties of one execution of a command (passed by {CmdConsole#run_command} as a hash of
    ...
    def _pry_
      CmdConsole::Warning.warn('_pry_ is deprecated, use pry_instance instead')
      pry_instance
    end
    ...
    def pass_block(arg_string)
      ...
      block_string =
        if !CmdConsole::Code.complete_expression?(prime_string)
      ...
      rescue CmdConsole::RescuableException
