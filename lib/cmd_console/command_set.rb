...
class CmdConsole
  ...
  class CommandSet
    ...
    # Defines a new CmdConsole command.
    ...
    #   command parameters are separated by whitespace at the CmdConsole prompt.
    # @example
    #   MyCommands = CmdConsole::CommandSet.new do
    ...
    def block_command(match, description = "No description.", options = {}, &block)
      ...
      options = CmdConsole::Command.default_options(match).merge!(options)

      @commands[match] = CmdConsole::BlockCommand.subclass(
        match, description, options, helper_module, &block
      )
    end
    alias command block_command

    # Defines a new CmdConsole command class.
    #
    # @param [String, Regexp] match The start of invocations of this command.
    # @param [String] description A description of the command.
    ...
    #
    # @example
    #   CmdConsole::Commands.create_command "echo", "echo's the input", :shellwords => false do
    #     def options(opt)
    ...
    #         raise CmdConsole::CommandError, "-u and -d makes no sense"
    #       end
    #       result = args.join(" ")
    ...
    def create_command(match, description = "No description.", options = {}, &block)
      ...
      @commands[match] = CmdConsole::ClassCommand.subclass(
        match, description, options, helper_module, &block
      )
    ...
    #   will be imported.
    # @return [CmdConsole::CommandSet] Returns the reciever (a command set).
    def import(*sets)
      sets.each do |set|
    ...
    def find_command_by_match_or_listing(match_or_listing)
      cmd = (@commands[match_or_listing] ||
        CmdConsole::Helpers::BaseHelpers.find_command(match_or_listing, @commands))
    ...
    #   CmdConsole.config.commands.alias_command "lM", "ls -M"
    # @example Pass explicit description (overriding default).
    #   CmdConsole.config.commands.alias_command "lM", "ls -M", :desc => "cutiepie"
    ...
    #   CmdConsole.config.commands.rename "dir", "ls", :description => "DOS friendly ls"
    ...
    #   end
    # @example Getting
    #   CmdConsole.config.commands.desc "amend-line"
    ...
    #
    # @param [CmdConsole::Command] command
    #   The command to add.
    #
    # @return [CmdConsole::Command]
    #   Returns the new command (matched with "pattern".)
    #
    # @example
    #   CmdConsole.config.commands["help"] = MyHelpCommand
    #
    def []=(pattern, command)
      ...
      unless command.is_a?(Class) && command < CmdConsole::Command
        raise TypeError, "command is not a subclass of CmdConsole::Command"
      end
    ...
    #   a subclass of CmdConsole::Command.
    #
