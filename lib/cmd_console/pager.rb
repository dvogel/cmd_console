...
class CmdConsole
  class Pager
    ...
    # `CmdConsole.config.pager` is enabled). If you want to send text through in
    ...
    # you must rescue `CmdConsole::Pager::StopPaging`. These requirements can be
    ...
    # comes in. Used when `CmdConsole.config.pager` is false.
    class NullPager
      def initialize(out)
        @out = out
      end
